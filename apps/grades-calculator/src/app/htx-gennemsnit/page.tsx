...
import { HtxVaegtningForklaretSection } from "@/src/components/site/htx-vaegtning-forklaret-section"
...
import {
  DEFAULT_HTX_ROWS,
  HTX_ROWS_COOKIE_NAME,
  HTX_SUBJECT_LABELS,
  HTX_SUBJECT_OPTIONS,
} from "@/src/lib/htx"

const SITE_URL = "https://www.gennemsnitsberegner.dk"
const PAGE_URL = `${SITE_URL}/htx-gennemsnit`
const TITLE = "HTX Gennemsnit – Beregn Dit Karaktergennemsnit På HTX"
const DESCRIPTION =
  "Gratis HTX gennemsnit beregner. Vælg fag og niveau (A/B/C), indtast dine karakterer, og få dit niveauvægtede gennemsnit inkl. bonus for A-niveau-fag."

export const metadata: Metadata = {
  title: TITLE,
  description: DESCRIPTION,
  alternates: {
    canonical: "/htx-gennemsnit",
  },
  openGraph: {
    title: TITLE,
    description: DESCRIPTION,
...
const HTX_FAQ_ITEMS = [
  {
    question: "Hvordan beregnes HTX gennemsnit?",
    answer:
  ...
  {
    question: "Hvad er Teknikfag?",
    answer:
      "Teknikfag dækker det A-niveau-teknikfag, der hører til din studieretning (fx Design B, Kommunikation/IT A eller lignende). Vælg 'Teknikfag' i fag-listen og indtast dine karakterer for netop dit teknikfag.",
  },
  {
    question:
      "Tæller studieområdeprojektet (SOP) med i gennemsnittet?",
    answer:
...
const faqJsonLd = {
  "@context": "https://schema.org",
  "@type": "FAQPage",
  mainEntity: HTX_FAQ_ITEMS.map((item) => ({
    "@type": "Question",
    name: item.question,
    acceptedAnswer: {
      "@type": "Answer",
...
export default async function HtxPage() {
  const cookieStore = await cookies()
  const raw = cookieStore.get(HTX_ROWS_COOKIE_NAME)?.value
    ...
    <div className="flex flex-1 flex-col bg-muted">
      ...
      >
        <h1 className="mb-2.5 text-balance break-words text-[26px] leading-[1.15] font-extrabold sm:text-[38px]">
          Beregn dit HTX gennemsnit
        </h1>
        <p className="mb-8 text-base leading-relaxed text-muted-foreground">
          Gratis beregner til dit HTX-eksamensbevis, forudfyldt med de
          obligatoriske fag. Tilføj dine studieretnings- og valgfag, indtast
          dine standpunkts- og eksamenskarakterer, og få dit niveauvægtede
      ...
      <RelatedCalculatorsSection currentSlug="htx" />
